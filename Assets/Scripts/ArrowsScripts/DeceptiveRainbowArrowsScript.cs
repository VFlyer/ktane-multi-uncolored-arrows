...
public class DeceptiveRainbowArrowsScript : BaseArrowsScript
{
    ...
    void CalculateQueries()
    {
        ...
        QuickLogFormat("Cross referencing this value with the second query's table should give you {0}.", ToOctal(curQuery, 2));
        selectedResponseValues.Add(responseValues[curQuery]);
        ...
        while (colorIdxes[(rotateDirectionIdx + curDirectionIdx) % 8] != textColorIdx && rotateDirectionIdx < 8)
            rotateDirectionIdx++;
        ...
        QuickLogFormat("Number of steps clockwise from white to the arrow same color as the the text flashing: {0}", rotateDirectionIdx);
        for (var p = 0; p < 4; p++)
        {
        ...
        curQuery = twoBitsTable[newValue];
        QuickLogFormat("Cross referencing the second query's table with the 2 digit concatenation of the remaining numbers should give you {0}.", ToOctal(curQuery, 2));
        selectedResponseValues.Add(responseValues[curQuery]);
        ...
        QuickLogFormat("Octal value: {0}", ToOctal(finalCalculatedValue, 2));
        QuickLogFormat("Cross referencing the final calculated value with the second query's table should give you {0}.", ToOctal(curQuery, 2));
    ...
    protected IEnumerator DelayQuery()
    {
        ...
        y = responseValues[y];
        QuickLogFormat("Queried {0}, showing response {1}.", colorIdxPressed.Select(a => debugColors[a]).Join(", "), ToOctal(y, 2));
    ...
    {
        Debug.LogFormat("[Deceptive Rainbow Arrows #{0}]: {1}", moduleId, string.Format(toLog, args));
    }
    protected override void QuickLog(string toLog = "")
    {
        Debug.LogFormat("[Deceptive Rainbow Arrows #{0}]: {1}", moduleId, toLog);
    }
    ...
    protected override IEnumerator ProcessTwitchCommand(string command)
    {
        ...
        else
        {
            ...
            for (int x = 0; x < cmdSets.Length; x++)
            {
                ...
                else if (Regex.IsMatch(cmdSets[x], @"^\s*(d(own)?[-\s]?r(ight)?|s(outh)?[-\s]?e(ast)?)\s*$", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant))
                {
                    allPresses.Add(arrowButtons[3]);
                }
                else if (Regex.IsMatch(cmdSets[x], @"^\s*(d(own)?[-\s]?l(eft)?|s(outh)?[-\s]?w(est)?)\s*$", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant))
                {
                    allPresses.Add(arrowButtons[5]);
                }
                else if (Regex.IsMatch(cmdSets[x], @"^\s*(u(p)?[-\s]?l(eft)?|n(orth)?[-\s]?w(est)?)\s*$", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant))
                {
